    ...
    <footer class="bg-slate-950 text-slate-500 py-12 border-t border-white/5">
        <div class="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div class="flex flex-col items-center md:items-end gap-1 text-xs">
                ...
                <p class="opacity-50">&copy; 2026 AGROIDEAS</p>
            </div>
        </div>
    </footer>
    `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (headerElement) headerElement.outerHTML = components.header;

    const footerElement = document.querySelector('footer');
    if (footerElement) footerElement.outerHTML = components.footer;
    ...
    const updateFooterDate = () => {
        ...
        if (dateEl) {
            ...
            dateEl.textContent = `${String(today.getDate()).padStart(2, '0')}/${String(today.getMonth() + 1).padStart(2, '0')}/${today.getFullYear()}`;
        }
    };
    updateFooterDate();
});
